...
- [ ] Sitemap needs nothing: `/sitemap.xml` is generated by `sitemap.ts` from `blog_posts.ts`, so there is no file to hand-edit (the stale root `sitemap.xml` was deleted)
