import { absolutePostUrl, BLOG_POSTS } from "./blog_posts.ts";
...
function blogEntries(lang: "en" | "es"): SitemapEntry[] {
  return BLOG_POSTS.filter((p) => p.lang === lang).map((p) => ({
    loc: absolutePostUrl(p.path),
    changefreq: "monthly" as const,
    priority: "0.8",
  }));
}

/** Sitemap URLs: static pages + blog posts from blog_posts.ts. */
